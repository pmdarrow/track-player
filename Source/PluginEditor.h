...
class PlaylistRowComponent final : public juce::Component {
 ...
 private:
  ...
  // or play the track here — that's the double-click behaviour — so the user
  // can move focus around (e.g. to target a Remove) without interrupting
  // playback.
  ...
  // Double click: "commit" to this track — load it into the transport and
  // start playback.
  void mouseDoubleClick(const juce::MouseEvent&) override {
    if (rowNumber >= 0 && onPlay) onPlay(rowNumber);
  }
  // Start a drag-and-drop operation once the mouse has moved far enough from
  // the initial press to distinguish it from a click or double-click. The
  // source description encodes the row index so the drop target knows which
  // track was picked up.
